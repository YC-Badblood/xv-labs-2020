 ...
 */
void kvminit()  // 初始化内核页表，由于还没有创建映射，所以都是直接映射
{
...
void kvmmap(uint64 va, uint64 pa, uint64 sz, int perm) // 将物理地址pa映射到虚拟地址va
{
  if (mappages(kernel_pagetable, va, sz, pa, perm) != 0)
    panic("kvmmap");  // 将mappages封装起来，如果分配页表失败，报错
}
...
int pgtblprint(pagetable_t pagetable, int depth){
  // there are 2^9 = 512 PTEs in a page table.
  for(int i = 0; i < 512; i++){ // 为何是512，因为页表项是9位，所以有512个页表项
    pte_t pte = pagetable[i];
    if(pte & PTE_V) { // 如果页表项有效
      ...
      // 如果该节点不是叶子节点，递归打印下一级页表
      if((pte & (PTE_R | PTE_W | PTE_X)) == 0){ // 若三个标志位都为0，说明该节点不是叶子节点
        // 叶子节点是肯定有这些属性其一的，要不然该节点就没有实际意义了
        uint64 child = PTE2PA(pte); // 指向下一个页表的物理地址
        pgtblprint((pagetable_t)child, depth + 1);  // 递归算法
      }
    }
  }
  return 0;
